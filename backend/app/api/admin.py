...
def dashboard(db: Annotated[Session, Depends(get_db)]) -> dict:
    ...
    return {
        ...
        "recent_users": [{"id": str(user.id), "email": user.email, "full_name": user.full_name, "role": user.role, "is_active": user.is_active, "created_at": user.created_at} for user in recent_users],
...
def users(page: int = 1, page_size: int = 25, search: str | None = None, db: Session = Depends(get_db)) -> dict:
    ...
    return {"items": [{"id": str(row.id), "email": row.email, "full_name": row.full_name, "role": row.role, "is_active": row.is_active, "created_at": row.created_at, "last_login_at": row.last_login_at} for row in rows], "page": page, "page_size": page_size, "total": db.scalar(count_query) or 0}
